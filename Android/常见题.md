...
- StringBuffer中很多方法可以带有synchronized。

##### 泛型
- 泛型: 通配符，可以用在类上了，接口，变量上，可以为限制内的所有参数。

##### 注解 
>- 介绍: 注解是一种特殊的接口,不能实现，对对象和参数、方法做标记作用。对东西啊加注解，通过反射读取来完成特定的任务。
>- 功能: 生成文档 ; 跟踪代码依赖性，实现替代配置文件功能 ; 在编译时进行格式检查。

- 四个元注解: 
	- @Documented: javadoc工具提取成文档.
	- @Inherited: 如果某个类使用了@Xxx注解（定义该Annotation时使用了@Inherited修饰）修饰，则其子类将自动被@Xxx修饰.
	- @Retention: 类型的注解保留:
		- SOUCRCE: 保留在源码中。
		- CLASS: 保留到字节码中。
		- RUNTIME: 运行时保留。
...
```
> 将setState里的对象和原来的对象进行合并，将新的状态存放到状态队列，这个状态队列并不会立刻更新，可能是需要等待像js机制，如果处理队列之前，还调用新的setState则会做批处理，之后就判断是否需要更新shouldComponetUpdate,是true，则进行更新，根据新的state生成新的元素树，再和老的元素树进行对比(diff)，最新差异化更新。
