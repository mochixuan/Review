...
##### 设计模式
- 单例模式: 将构造函数设置为私有，该类的私有静态变量，提供一个静态的方法去获取该变量。在单例模式中全局只有一个该类的实例，只有一个类节省资源，全局一个实例方便管理。缺点: 
...
- constructor(props)，构造函数,初始化state,访问props。
...
>- 删除了三个生命周期componentWillMount、componentWillUpdate、shouldComponentWillReceiveProps.16.x可以使用UNSAFE_,17后将会被移除。
